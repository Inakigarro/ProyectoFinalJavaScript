...
async function obtenerHorariosDelDiaActual() {
	let h = recuperarHorariosDesdeLocalStorage();
	if (h.length === 0) {
		await recuperarHorariosDesdeDb();
		h = recuperarHorariosDesdeLocalStorage();
	}
...
function marcarHorariosNoDisponibles() {
	// Busco los turnos.
	let arrayTurnos = obtenerTodosTurnos();
	// Recorro los turnos y busco los nodo horas correspondientes a los turnos.
	arrayTurnos.forEach((t) => {
		let nodoHorario = document.getElementById(
			`nodoHora${t.cancha.id}${t.horario.id}`
		);
		nodoHorario.classList.add("deshabilitado");
	});
}

function generarListaHorarios(idCancha) {
	let listaHorarios = crearNodo("ul", "listaHorTemp", null, null, [
		"list-group",
		"list-group-flush",
	]);
	...
	horarios.forEach((horario) => {
		...
		if (listaHoras) {
			let nodoHora = crearNodoHijo(
				"li",
				`nodoHora${idCancha}${horario.id}`,
				null,
				`${horario.horaInicio} - ${horario.horaFin}`,
				["list-group-item", "botonHora"],
				listaHoras
			);
			...
			let nodoDia = crearNodo(
				"li",
				null,
				`<h5 class="card-header">${horario.dia}</h5>`,
				null,
				null
			);
			let listaHoras = crearNodoHijo(
				"ul",
				`cancha${idCancha}${horario.dia}`,
				null,
				null,
...
async function renderizarCanchas() {
	...
	await obtenerCanchasIniciales();
	await obtenerHorariosDelDiaActual();
