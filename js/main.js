...
function seleccionDeCancha(canchasDisponibles) {
    // Solicito la seleccion de una cancha para jugar.
	canchaSeleccionada =
		parseInt(prompt(`Seleccione una de las siguientes canchas disponibles para jugar:
${canchasDisponibles}`));

    // Si la cancha seleccionada no se encuentra entre las disponibles
    // Vuelvo a solicitar.
	while (!canchasDisponiblesArray.includes(canchaSeleccionada)) {
		canchaSeleccionada =
		parseInt(prompt(`Seleccione una de las siguientes canchas disponibles para jugar:
${canchasDisponibles}`));
	}
...
function seleccionDeturno(turnosDisponibles) {
	...
	turnoSeleccionado = parseInt(
		prompt(`Seleccione una de las siguientes canchas disponibles para jugar:
${turnosDisponibles}`)
	);
    
    // Si el turno seleccionado no se encuentra entre los disponibles
    // Vuelvo a solicitar.
    while (!turnosDisponiblesArray.includes(turnoSeleccionado)) {
        turnoSeleccionado = parseInt(
        prompt(`Seleccione una de las siguientes canchas disponibles para jugar:
${turnosDisponibles}`)
        );
